...
Func CreateMainWindow()
    ...
    Global $g_btnImportXLS = GUICtrlCreateButton("Import Excel...", 10, $APP_HEIGHT - 58, 120, 32)
    GUICtrlSetOnEvent($g_btnImportXLS, "_ImportExcel")
    GUICtrlSetFont(-1, 9, 400, 0, "Segoe UI")

    Global $g_btnExportXLS = GUICtrlCreateButton("Export Excel...", 138, $APP_HEIGHT - 58, 120, 32)
    GUICtrlSetOnEvent($g_btnExportXLS, "_ExportExcel")
    GUICtrlSetFont(-1, 9, 400, 0, "Segoe UI")

    Global $g_btnLoadDB = GUICtrlCreateButton("⬇ Load from DB", 266, $APP_HEIGHT - 58, 130, 32)
    ...
    Global $g_btnClearDB = GUICtrlCreateButton("Clear DB", 404, $APP_HEIGHT - 58, 100, 32)
    GUICtrlSetOnEvent($g_btnClearDB, "_ClearDatabase")
    GUICtrlSetFont(-1, 9, 400, 0, "Segoe UI")
    GUICtrlSetBkColor($g_btnClearDB, 0xFFDDDD)

    Global $g_btnGenerate = GUICtrlCreateButton("GENERATE SQL", 512, $APP_HEIGHT - 58, 125, 32)
    GUICtrlSetOnEvent($g_btnGenerate, "_GenerateSQL")
    GUICtrlSetFont(-1, 9, 700, 0, "Segoe UI")

    Global $g_btnExecute = GUICtrlCreateButton("RUN ON DB", 645, $APP_HEIGHT - 58, 130, 32)
    GUICtrlSetOnEvent($g_btnExecute, "_ExecuteSQL")
    GUICtrlSetFont(-1, 9, 700, 0, "Segoe UI")
    GUICtrlSetBkColor($g_btnExecute, 0xDDFFDD)

    Global $g_btnSaveSQL = GUICtrlCreateButton("Save SQL...", 783, $APP_HEIGHT - 58, 110, 32)
...
Func _LayoutMain($w, $h)
    ...
    If IsDeclared("g_btnImportXLS") Then GUICtrlSetPos($g_btnImportXLS, 10,  $yBtn, 120, 32)
    If IsDeclared("g_btnExportXLS") Then GUICtrlSetPos($g_btnExportXLS, 138, $yBtn, 120, 32)
    If IsDeclared("g_btnLoadDB")    Then GUICtrlSetPos($g_btnLoadDB,    266, $yBtn, 130, 32)
    If IsDeclared("g_btnClearDB")   Then GUICtrlSetPos($g_btnClearDB,   404, $yBtn, 100, 32)
    If IsDeclared("g_btnGenerate")  Then GUICtrlSetPos($g_btnGenerate,  512, $yBtn, 125, 32)
    If IsDeclared("g_btnExecute")   Then GUICtrlSetPos($g_btnExecute,   645, $yBtn, 130, 32)
    If IsDeclared("g_btnSaveSQL")   Then GUICtrlSetPos($g_btnSaveSQL,   783, $yBtn, 110, 32)
